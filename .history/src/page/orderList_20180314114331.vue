...
export default {
  ...
  methods: {
    ...
    async orderDetail(row){
      ...
      const shopping_address = await getShoppingAddress(row.user_id);//获取收货地址列表

      const itemData = {...row,...{restaurant_address:restaurant.address,restaurant_name:restaurant.name,user_name:shopping_address.name,user_address:shopping_address.address}}
    },
  }
}
</script>
